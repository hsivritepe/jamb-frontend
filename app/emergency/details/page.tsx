...
export default function EmergencyDetails() {
  ...
  const handleActivityToggle = (service: string, activity: string) => {
    setSelectedActivities((prev) => {
      ...
      if (serviceActivities[activity]) {
        ...
        // Select the activity with default quantity of 1
        return {
          ...prev,
  ...
  const handleQuantityChange = (service: string, activity: string, quantity: number) => {
    setSelectedActivities((prev) => ({
      ...prev,
      [service]: {
        ...prev[service],
  ...
  // Flatten the selected services into a single array
                      ...
                      return (
                        <div key={activityKey} className="space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-base text-gray-800">{activityLabel}</span>
                          ...
                          {isSelected && (
                            <div className="flex items-center gap-2">
                              <input
                                type="number"
                                min="1"
                                value={selectedActivities[service][activityKey]}
                                onChange={(e) =>
                              ...
                              <span className="text-sm text-gray-600">
                                {activityData.unit_of_measurement}
                              </span>
                            </div>
                          )}
                        </div>
